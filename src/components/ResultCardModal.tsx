...
export function ResultCardModal({ card, onClose }: ResultCardModalProps) {
  // Safe default: if card is null (shouldn't happen with parent AnimatePresence), use 'other'
  // But we technically expect card to be present when this component renders.
  const styles = card ? (cardTypeColors[card.card_type] || cardTypeColors.other) : cardTypeColors.other;
  ...
  // If card is mysteriously null, render nothing (but this shouldn't happen during exit if set up right)
  if (!card) return null;
