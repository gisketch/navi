...
// ============================================
// Overlay shown when using voice in Finance tab
// - Gradient dim from bottom to top (no blur)
// - Navi repositions to center (handled by App.tsx)
// - Shows live transcription with ChatUI-style bubbles

interface FinanceVoiceOverlayProps {
  isOpen: boolean;
...
export function FinanceVoiceOverlay({
  isOpen,
  currentTurn,
  liveStatus,
  isRecording = false,
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        ...
        >
          {/* Backdrop with blur and gradient dim */}
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            ...
            style={{
              background: 'linear-gradient(to top, rgba(0,0,0,0.85) 0%, rgba(0,0,0,0.6) 40%, rgba(0,0,0,0.3) 70%, rgba(0,0,0,0.2) 100%)',
            }}
          />
